...
Fix bug in JS that caused problems with forms with inputs named "method", "action" or "target".

=back

=head2 3.27 - Aug 16, 2020

=over

=item *

Added new C<may_skip_related_assets> group permission flag

=item *

Added new "Skip Related Assets" button during publish to be used with caution
(and only with the C<may_skip_related_assets>).
...
Upgraded Scalar-List-Utils package to 1.55

=item *

Load AddOns InitHandler in krang_apachectl so modules can be overridden

=back

=head2 3.26 - June 28, 2020

=over

=item *

Added CDN support for media objects. This includes a new C<EnableCDNSupport> config
directive, C<cdn_url> on sites and C<cdn_enabled> on media objects. When all
three are set, allows the URL for a media object during publishing to use a CDN
hostname instead of the base site.

=item *

Fixed media import/export bug where C<full_text> was not saved.

=item *

Add F<krang_upgrade_instances> script to upgrade instances in-place to handle
the case where you migrating from a different installation to a new one with a
different version.

=back

=head2 3.25 - June 15, 2020

=over

=item *

Fix last modification date in Krang FTP listings

=item *

Add build support for RHEL/CentOS 6/7

=item *

Upgrade Apache/mod_perl/mod_ssl and lots of Perl modules to work with RHEL/CentOS 6/7

=item *

Remove any legacy CVS bits

=item *

Support MySQL Multi-byte UTF8 (emojis)

=item *

Upgrade all MySQL tables to use InnoDB and have DYNAMIC row layout (to support
new MySQL features like Multi-Byte UTF8)

=item *

Remove lots of deprecated Perl usages

=item *

Allow F<krang_build> to build a single module

=item *

Allow more customization of forgot-pw email template by addons

=item *

Added SafeStopSeconds and /krang-ping URL to make it easier to manage Krang instances behind a Load Balancer
